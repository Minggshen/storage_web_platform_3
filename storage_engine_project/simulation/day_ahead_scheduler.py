...
class DayAheadScheduler:
    ...
    def _build_cache_key(
        ...
        return (
            ...
            round(float(effective_power_cap_kw), 4),
            round(initial_soc, 4),
            None if terminal_soc_target is None else round(terminal_soc_target, 4),
            round(float(soc_min), 6),
            round(float(soc_max), 6),
            bool(op_cfg.enable_transformer_limit),
            None if transformer_limit_kw is None else round(float(transformer_limit_kw), 4),
            bool(op_cfg.enforce_daily_terminal_soc),
            round(float(op_cfg.daily_terminal_soc_tolerance), 6),
            round(float(strategy.eta_charge), 6),
            round(float(strategy.eta_discharge), 6),
            round(float(strategy.degradation_cost_yuan_per_kwh_throughput), 6),
            bool(strategy.allow_grid_charging),
            round(float(cfg.terminal_soc_penalty_yuan_per_unit_sq), 6),
            round(float(cfg.transformer_violation_penalty_yuan_per_kwh), 6),
            round(float(cfg.throughput_penalty_yuan_per_kwh), 6),
            round(float(cfg.smoothness_penalty_yuan_per_kw_change_sq), 9),
            round(float(cfg.service_degradation_weight), 6),
            ...
            tuple(np.round(svc_profile.capacity_price_yuan_per_kw, 6)),
            tuple(np.round(svc_profile.delivery_price_yuan_per_kwh, 6)),
            tuple(np.round(svc_profile.penalty_price_yuan_per_kwh, 6)),
            round(float(svc_profile.max_service_power_kw), 6),
            round(float(svc_profile.headroom_ratio), 6),
            round(float(svc_profile.expected_penalty_ratio), 6),
        )
